(()=> {
  ...
  function newGame(resetScore=false){
    ...
    current = 1; over = false; moves = []; redoStack = [];
...
current = startP1.checked ? 1 : 2; // Définit qui commence (UNE seule fois)
  ...
  function handleTurn(c){
    if(over) return;
    const r = firstEmptyRow(c); if(r<0) return bumpColumn(c);
    ...
    if(win){
      celebrate(win);
      scores[current]++; updateScore();
      statusEl.textContent = `Victoire du Joueur ${current} ${current===1?'🔴':'🟡'} !`;
      over = true; sound('win'); return;
    }
    if(isDraw()){ statusEl.textContent='Match nul. GG à vous deux.'; over=true; sound('drop'); return; }

    current = 3-current;
    updateStatus();
    if(modeAi.checked && current===2){ setTimeout(()=> aiPlay(), 320); }
  }

  function updateStatus(){
    const txt = current===1? 'À toi de jouer, Joueur 1 🔴' : (modeAi.checked? "L'IA réfléchit… 🤖" : 'À toi de jouer, Joueur 2 🟡');
  ...
  function bumpColumn(c){
    ...
    cells.forEach(el=> el.animate([{transform:'translateY(0)'},{transform:'translateY(-3px)'},{transform:'translateY(0)'}],{duration:160, easing:'ease-out'}));
    sound('bump');
  }
  ...
  function placeWithoutAnim({r,c,player}){
    ...
    const token=document.createElement('div'); token.className=`token p${player}`; token.style.transform='translateY(0)'; cell.appendChild(token);
  }
  ...
  function aiPlay(){
    ...
    if(col==null){ const valid = validColumns(grid); const pick = valid[Math.floor(Math.random()*valid.length)]; handleTurn(pick); }
    else handleTurn(col);
  }
  function validColumns(g){ const v=[]; for(let c=0;c<COLS;c++) if(g[0][c]===0) v.push(c); return v; }
  ...
  function scorePosition(g, player){
    ...
    let centerCount=0; for(let r=0;r<ROWS;r++) if(g[r][Math.floor(COLS/2)]===player) centerCount++; score += centerCount*3;
    ...
    for(let r=0;r<ROWS;r++) for(let c=0;c<COLS-3;c++) evalWindow([g[r][c],g[r][c+1],g[r][c+2],g[r][c+3]]);
  ...
  function minimax(g, depth, alpha, beta, maximizing){
    ...
    if(maximizing){
      ...
      for(const col of valid){
        ...
        const {ng} = sim; const result = minimax(ng, depth-1, alpha, beta, false);
      ...
      for(const col of valid){
        ...
        const {ng} = sim; const result = minimax(ng, depth-1, alpha, beta, true);
        if(result.score < value){ value = result.score; bestCol = col; }
